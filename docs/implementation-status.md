...
> Snapshot: 2026-08-25，base HEAD `ab16e65` 加上当前未提交工作树变更；`/mcp` 单一 Streamable HTTP endpoint 已完成双架构 Debug 构建、完整部署与 x64/x86 live MCP 路由验收。此前 `disassemble` import 符号化、breakpoints Resource 与 `memory{action:"read"}` 迁移的 x64 专项验收完成于 2026-08-14；breakpoints Tool 的首批 mutation 与统一 `get/disable/delete` 于 2026-08-17 完成 x64 live MCP 专项验收和 x86 build-only 验证；`debug_control{action:"attach"}`、Resource namespace migration 与 attach candidates 于 2026-08-20 分别完成 x64 live MCP + NtObjectManager 闭环和 x86 build-only 验证。`DebugGUI` 已实现并完成 x64 live MCP 验收，x86 已完成构建与部署检查；三类 CPU pane 的 x86 GUI 行为仍待 live 验收。新增的 module strings Resource 已完成 x64 live MCP 验证，x86 完成 Debug 构建与部署检查。统计先以实时 MCP `tools/list`、`resources/list`、`resources/templates/list` 为准，再与 `x64dbgMCP/x64dbgHandler.h` 中的注册和实现交叉核对；不是从根级 README 的产品清单反推。
...
实时 `resources/templates/list` 返回 8 个 Resource Templates：`x64dbg://modules{?offset,limit}`、`x64dbg://modules/{name}`、`x64dbg://modules/{name}/sections`、`x64dbg://modules/{name}/exports`、`x64dbg://modules/{name}/imports`、`x64dbg://modules/{name}/strings{?offset,limit,length}`、`x64dbg://breakpoints{?offset,limit}`、`x64dbg://attach/processes{?offset,limit}`。因此当前共有 16 个可读取 Resource 端点，但只有前 8 个出现在固定 Resource 列表中。

当前固定 Resource `x64dbg://session/debuggee` 的 MCP metadata name 为 `session-debuggee`；URI 不变，仍属于固定 Resource 而非 Resource Template。其它导航根跨资源指向该 URI 时统一使用 `_links.session_debuggee`；该 Resource 自身的 canonical self link 仍为 `_links.self`。当前部署物的 x64 `resources/list` 已返回该 metadata name，x64/x86 `x64dbg://session` live read 均返回 `_links.session_debuggee`。
...
当前里程碑实现 [ADR-008](adr/008-mcp-endpoint-path.md)：Kestrel 仍监听 `http://localhost:3001`，唯一 MCP endpoint 改为 `http://localhost:3001/mcp`，根路径不保留 alias；Streamable HTTP 显式使用 stateless mode 并关闭 Legacy SSE。携带 `Origin` 的请求只允许 loopback HTTP(S) origin；无 `Origin` 的非浏览器 MCP 客户端不受影响。

此前里程碑实现 [ADR-006](adr/006-debug-gui-evidence-capture.md) 的 `debug_gui{snapshot,focus,get,set}`。`snapshot` 在 x64dbg GUI thread 刷新并捕获完整主窗口，通过 WIC 编码为 PNG；省略 `save_path` 时返回 typed JSON text + MCP image block，提供绝对新 `.png` 路径时只创建文件并返回 metadata/path。`focus/get/set` 把 CPU pane 导航、选区、聚焦、刷新、event flush 与回读组合成单个 GUI-thread transaction；`set` 同时返回 resolved requested 和 GUI actual range。截图摘要是 PNG byte-identity anchor，不是签名 provenance 或可信时间戳。
...
当前 endpoint 专项验收分别启动最终部署的 x64dbg PID 41504 与 x32dbg PID 13596。两者均由各自进程监听 3001，完成相同路由矩阵后退出并释放端口；这次验收不启动 debuggee，因此 Tool 执行只验证 `disassemble(addr=0,count=1)` 的 contracted `not_attached` 返回，不重复声明调试器领域行为。

| Check | Status | Evidence / limit |
|---|---|---|
| Debug x64 build | Verified | `out/bin/x64-Debug/x64dbgMCP.dp64`, SHA-256 `65C20DD9130BEBBB3F6F8A10EDACFE516027F7778261D685AD0558BFC5BE284C`; 2026-08-25 CMake `deploy_x64dbg_plugin` run succeeded with 0 errors and the known C4945 duplicate-import / C5307 literal-conversion warnings |
| Debug Win32 build | Verified | `out/bin/win32-Debug/x64dbgMCP.dp32`, SHA-256 `10F3EC64A785CADA1C0B8EBE9026C1524AA4D586A9853A02CBE0D03DAC39EDB9`; the same run succeeded with 0 errors and the known C4945 warnings; DebugGUI and module strings remain live-unverified on x86 |
...
| `/mcp` transport | Verified on x64 and x86 | For both deployed architectures, `POST /mcp` initialized with 200, `POST /` returned 404, `GET /mcp` returned 405, and `/mcp/sse` plus `/mcp/message` returned 404. A hostile `Origin: https://evil.example` returned 403 while loopback origins initialized with 200. `mcporter` reported `status=ok` for `HTTP http://localhost:3001/mcp`, enumerated the Tool catalog, read Resources, and invoked `disassemble` to the contracted `not_attached` envelope. |
| MCP catalog | Verified on current x64 and x86 | Both deployed architectures returned exactly `debug_control`, `breakpoints`, `registers`, `disassemble`, `debug_gui`, `memory`, `logging`. Current x64 `resources/list` returned all 8 fixed Resources, and direct `resources/templates/list` returned all 8 templates including `x64dbg://modules/{name}/strings{?offset,limit,length}`. Current x64/x86 `x64dbg://session` reads reported their matching platform and the scoped navigation links. |
| Scoped debuggee naming | Live verified on current deployment | x64 `resources/list` returned metadata `name="session-debuggee"` for URI `x64dbg://session/debuggee`; x64/x86 `x64dbg://session` reads both returned `_links.session_debuggee`. The Resource itself and VS Code client UI were not reread in this endpoint-only matrix. |
...
| `x64dbg://modules/{name}/strings` | Implemented; x64 live verified against CSV | Final CMake-deployed x64 Notepad session scanned `0x7FF711030000`/`0x5A000`; `length=3` returned 679 rows. Compared with the external baseline `C:\Users\r0th3r\Downloads\Tools\x64dbg\release\x64\db\export-20260825-160413.csv`, 650 CSV reference rows had 639 matching `(instructionAddress,stringAddress)` keys (98.31%); after normalizing address/spacing/operand-size display, 633/639 common disassembly rows matched (99.06%), with six symbolized `&jmp.&_o__purecall` CSV rows differing only because this Resource returns fast-disassembly numeric targets. At the unique string-address level, 262/270 addresses were shared and 261/262 shared contents matched after normalizing x64dbg display quoting/escaping. The sole content difference was mutable process data at `0x7FF711064400`. The 11 missing rows were all short two/three-character candidates below the Resource's `length=3` threshold; 40 extra rows were tolerated scanner/heuristic differences. The sample `0x7FF71103316F` → `0x7FF71105AA90` decoded as strict UTF-8 `onecore\\internal\\sdk\\inc\\wil\\opensource\\wil\\resource.h`. `length=2`, `513`, and `abc` were rejected; pages `0/5` and `5/5` preserved totals and next/prev links. x86 was build/deployment checked only. |
...
| Release builds | Verified for current source | 2026-08-21 x64 and Win32 Release builds both completed with 0 errors and the known C4945 duplicate-import warnings. `x64-Release/x64dbgMCP.dp64` SHA-256 is `0B9870245A0463C71B82545E1CDB9DA53C43C77EEB1A0459A9E7B3CF46CF83DF`; `win32-Release/x64dbgMCP.dp32` SHA-256 is `812EA78FB6E7F2E4F15EE79B414FEA2380A26A28C165703D619AB6F19BFD113E`. Release artifacts were not deployed or live-run. |
