...
当前源代码契约中，固定 Resource `x64dbg://session/debuggee` 的 MCP metadata name 为 `session-debuggee`；URI 不变，仍属于固定 Resource 而非 Resource Template。其它导航根跨资源指向该 URI 时统一使用 `_links.session_debuggee`；该 Resource 自身的 canonical self link 仍为 `_links.self`。这一命名变更已完成双架构 Debug build，尚未重新部署并完成 VS Code live MCP 验收。
...
| Scoped debuggee naming | Build verified; live pending | Source registers Resource metadata `name="session-debuggee"` and cross-resource `_links.session_debuggee` while preserving URI `x64dbg://session/debuggee`; x64 Debug and Win32 Debug builds passed. The rebuilt artifacts have not been deployed for a post-change `resources/list` / `resources/read` / VS Code verification. |
