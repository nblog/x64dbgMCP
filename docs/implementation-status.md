...
|---|---|---|
| Debug x64 build | Verified | `out/bin/x64-Debug/x64dbgMCP.dp64`, SHA-256 `7F2F77E5BA5BEF403EDC09E38AFAC3E491D76D2324408A9930960765029017CF`; 2026-08-21 CMake/MSBuild run succeeded with 0 errors and the known C4945 duplicate-import warnings |
| Debug Win32 build | Verified | `out/bin/win32-Debug/x64dbgMCP.dp32`, SHA-256 `0F68A6BE74D5FA67DEEC309F1BCEC59466BF1655F55D13FBDA72F187C36BEEA9`; the same run succeeded with 0 errors and the known C4945 warnings; DebugGUI remains live-unverified on x86 |
...
| MCP catalog | Verified on current x64 and x86 | Both deployed architectures returned exactly `debug_control`, `breakpoints`, `registers`, `disassemble`, `debug_gui`, `memory`, `logging`. Current x64 `resources/list` returned all 8 fixed Resources, including metadata name `session-debuggee`; current x64/x86 `x64dbg://session` reads reported their matching platform and the scoped navigation links. Resource templates and unrelated payload matrices were not rerun for this endpoint-only milestone. |
