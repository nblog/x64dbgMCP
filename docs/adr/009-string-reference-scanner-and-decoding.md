...
MCP 需要可重复的结构化 Resource 结果，不能依赖 GUI Reference View 的全局状态。因此本项目为模块与未来 memory Resource 共享一个按地址范围工作的扫描抽象。第一批暴露 `x64dbg://modules/{name}/strings`，后续 `x64dbg://memory/{address}/strings` 只负责解析范围并复用同一抽象。
...
`rangeSize` 与 `minLength` 是两个不同维度。模块 Resource 默认把模块 `base/size` 作为扫描范围；其 URI 的 `length` 参数表示 `minLength`，而不是只读取模块前 4 个字节。解码器会从目标字符串地址读取至 NUL 或内部最大探测长度（512 字节），因此不会把默认值 4 误用成字符串截断长度。未来 memory Resource 应继续单独解析 `rangeSize`，并复用同一个 `minLength` 语义。
...
- 模块和未来 memory Resource 可以共享解码、缓存、反汇编操作数处理和分页前的结果生成逻辑。
