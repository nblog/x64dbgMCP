...
> Snapshot: 2026-08-20，base HEAD `0bcd0f6`；最近一次双架构 live MCP 验收完成于 2026-08-09，`disassemble` import 符号化、breakpoints Resource 与 `memory{action:"read"}` 迁移的 x64 专项验收完成于 2026-08-14；breakpoints Tool 的首批 mutation 与统一 `get/disable/delete` 于 2026-08-17 完成 x64 live MCP 专项验收和 x86 build-only 验证；`debug_control{action:"attach"}` 于 2026-08-20 完成 x64 live MCP + NtObjectManager 闭环和 x86 build-only 验证。统计先以实时 MCP `tools/list`、`resources/list`、`resources/templates/list` 为准，再与 `x64dbgMCP/x64dbgHandler.h` 中的注册和实现交叉核对；不是从根级 README 的产品清单反推。
...
当前里程碑为既有 `debug_control` 增加 `attach` action，不新增进程列表 Resource。`pid <= 0` 在 side effect 前返回 `invalid_argument`；detached 状态通过同步 `DbgCmdExecDirect("attach .<decimal-pid>")` 启动 attach loop。活跃调试会话下，省略或显式 `detach2attach=false` 均在 side effect 前拒绝；显式 `true` 时仅在本次同步命令分派期间把全局 `Engine/DetachOnAttach` 临时设为 `1`，随后恢复原设置，从而 detach 旧 debuggee、保持其运行并 attach 新 PID。本插件的并发 attach 调用由 mutex 串行化；Tool 的 success 只表示命令 handler 已接受并启动 attach，最终目标身份继续由 `x64dbg://session` 与 `x64dbg://process` 确认。此前里程碑包括 `breakpoints{get,set,set_hardware,disable,delete}`、`memory_read` → `memory{action:"read"}` 迁移和分页 `x64dbg://breakpoints` Resource。
...
当前 attach 专项验收使用 x64dbg attach 两个独立的 Windows Notepad 11.2606.15.0 x64 进程，并以 MCP Resources 和 NtObjectManager 双重确认目标身份、位数、Session 与存活状态。此前 memory 迁移与 breakpoints Resource/Tool 专项验收均使用 x64dbg 调试 Electron 32.2.8 x64；完整双架构基线还包括 x32dbg 调试 `%SystemRoot%\SysWOW64\notepad.exe`。所有 breakpoints Tool 状态变更均在 Tool 返回后用 `x64dbg://breakpoints` 二次确认。

| Check | Status | Evidence / limit |
|---|---|---|
| Debug x64 build | Verified | `out/bin/x64-Debug/x64dbgMCP.dp64`, SHA-256 `3452D2C846C29A0E4DD1456268B0952AF96AF18D0FCEE2A8CA16B8364FF930E7`, 2026-08-20 CMake/MSBuild build succeeded with 0 errors (8 pre-existing C4945 reference warnings on the compiling run) |
| Debug Win32 build | Verified | `out/bin/win32-Debug/x64dbgMCP.dp32`, SHA-256 `B2D28842D5216EDC3C25240A22092B8D5FDDBBDA2BB5BB786466B46D7D9AE819`, CMake/MSBuild build succeeded with 0 errors (same 8 C4945 warnings); current attach behavior is build-only on x86 |
| Full plugin deployment | Verified | Current x64 complete 11-file `OutputPath` set deployed to the specified x64dbg `plugins` directory with every source/deployed SHA-256 pair matched; deployed `.dp64` hash is `3452D2C846C29A0E4DD1456268B0952AF96AF18D0FCEE2A8CA16B8364FF930E7`; x86 was build-only in this milestone |
| MCP catalog | Verified on current x64 | Returned exactly `debug_control`, `breakpoints`, `registers`, `disassemble`, `memory`, `logging`. Live `debug_control` Schema required only `action`, advertised `attach`, exposed direct optional integer `pid` and boolean `detach2attach` with `default:false`. Existing `breakpoints` actions/schema and 8 fixed plus 6 template Resources remained registered; no process-list Resource was added |
...
| `debug_control` | Verified on current x64 | `attach(pid=0)` returned `invalid_argument` and left the session detached. Detached attach with omitted `detach2attach` selected Notepad A PID 11064. While A was active, both omitted and explicit `false` attempts for B returned `invalid_argument`, and `x64dbg://process` remained PID 11064. Explicit `true` switched the Resource to B PID 36760; NtObjectManager proved A remained alive, B was the active x64 target, both were Session 10, and the original `Engine/DetachOnAttach=0` was restored. Previous `init/run/pause/stop/StepInto/StepOver/StepOut/run_command(wait=true)` and x32 lifecycle evidence is retained |
...
| Server stop / cleanup | Verified for current attach run | Final `debug_control{stop}` was followed by `x64dbg://session.isDebugging=false` and `x64dbg://process.processId=0`; tracked B exited while detached A remained alive. Only test-recorded Notepad PIDs were then closed/terminated after path and creation-time verification. Closing tracked x64dbg PID 18564 removed its `[::1]:3001` listener; no tracked test PID remained. Occupied-port failure propagation was not rerun |
